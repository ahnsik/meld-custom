...
class ActionIcons:
    ...
    @classmethod
    def load_custom_copy(cls, direction: Gtk.IconLookupFlags):
        icon_name = (
            'button_copy1.png'
            if direction == Gtk.IconLookupFlags.DIR_LTR
            else 'button_copy0.png'
        )
        cache_key = f'custom-{icon_name}'
        icon = cls.icon_cache.get(cache_key)

        if not icon:
            icon_path = Path(__file__).with_name(icon_name)
            icon = GdkPixbuf.Pixbuf.new_from_file_at_scale(
                str(icon_path), -1, cls.pixbuf_height, True)
            cls.icon_cache[cache_key] = icon
...
class ActionGutter(Gtk.DrawingArea):
    ...
    def icon_direction_set(self, direction: Gtk.IconLookupFlags):
        ...
        self.action_map = {
            ...
            ActionMode.Insert: ActionIcons.load_custom_copy(direction),
        }
