...
class FileDiff(Gtk.VBox, MeldDoc):
    ...
    def __init__(
        ...
        *,
        ...
        actions = (
            ...
            ('file-push-left-next', self.action_push_change_left_next),
            ('file-push-right-next', self.action_push_change_right_next),
    ...
    def on_current_diff_changed(self, *args):
        ...
        self.set_action_enabled('file-push-left-next', push_left)
        self.set_action_enabled('file-push-right-next', push_right)
    ...
    def _push_change_and_go_next(self, direction):
        if self.num_panes == 2:
            src, dst = (1, 0) if direction == PANE_LEFT else (0, 1)
        else:
            src, dst = self.get_action_panes(direction)

        self.replace_chunk(src, dst, self.get_action_chunk(src, dst))

        if self.focus_pane:
            self.on_cursor_position_changed(
                self.focus_pane.get_buffer(), None, True)
        if self.cursor.next is not None:
            self.go_to_chunk(self.cursor.next)

    def action_push_change_left_next(self, *args):
        self._push_change_and_go_next(PANE_LEFT)

    def action_push_change_right_next(self, *args):
        self._push_change_and_go_next(PANE_RIGHT)
